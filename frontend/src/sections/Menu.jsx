           ...
           {previewItems[activeCategory].map((item) => (
             <div key={item.id} className="menu-item">
               ...
               <div className="menu-text">
                 ...
                 <p>{item.desc}</p>
                 {user?.rol === '1' ? (
                   <button className="btn-order" onClick={() => handleOrder(item)}><ShoppingBag size={16} /> Pedir</button>
                 ) : (
                   <p className="msg-login-order">Inicia sesión como cliente para pedir.</p>
                 )}
               </div>
             </div>
           ))}
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              {Object.entries(previewItems).map(([category, items]) => (
